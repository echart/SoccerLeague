...
class PlayerFactory{
	...
	public static function savePlayer(Player $player){
		...
		try{
			...
			$query->bindParam(":injury_propensity",$player->injury_propensity);
		...
		try{
			$query=Connection::getInstance()->connect()->prepare("INSERT INTO players_attr_line(id_player, crossing, pass, technical, ballcontrol, dribble, longshot, finish, heading, freekick, marking, tackling) values
			(:id_player,:crossing,:pass,:technical,:ballcontrol,:dribble,:longshot,:finish,:heading,:freekick,:marking,:tackling)");
			$query->bindParam(":id_player",$id_player);
