...
class Feed{
  ...
  public function __getLastTweetsByClub($qtd=20, $page=0){
    $offset=$page*$qtd;
    $query=Connection::getInstance()->connect()->prepare("SELECT id_tweet FROM tweet where id_club=:id_club and reply_to is null order by tweetdate desc limit :qtd offset :offset1");
